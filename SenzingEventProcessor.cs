...
using Ems.Domain.Models.Entities;
using Ems.Domain.Models.Records;
using Ems.EntityResolutionSource.Senzing.Interfaces;
using Ems.EntityResolutionSource.Senzing.Models.Typedef;
using Senzing.Sdk;
using static LanguageExt.Prelude;

namespace Ems.EntityResolutionSource.Senzing.Impl;
...
public sealed class SenzingEventProcessor : IEntityResolutionEngineEventProcessor
{
    ...
    public async Task<IReadOnlyList<EntityChangeSummary>> ProcessAffectedEntities(Seq<long> affectedEntityIds)
    {
        ...
        var currentByRecord = new Dictionary<RecordId, System.Collections.Generic.HashSet<long>>();
        ...
        var nextEntities = new Dictionary<long, System.Collections.Generic.HashSet<long>>();
        ...
        foreach (long entityId in affectedEntityIds)
        {
            ...
            var nextSet = new System.Collections.Generic.HashSet<long>();
        ...
        var contributors = new Dictionary<long, System.Collections.Generic.HashSet<long>>();
        ...
        foreach (var (currentEntityId, oldContributingEntityIds) in nextEntities)
        {
            foreach (long oldContributingEntityId in oldContributingEntityIds)
            {
                ...
                {
                    set = new System.Collections.Generic.HashSet<long>();
        ...
        foreach (long entityId in affectedEntityIds)
        {
            ...
            else if (didEntityExistBefore && doesEntityExistNow)
            {
                ...
                var oldSet = new System.Collections.Generic.HashSet<RecordId>(
                    beforeRecordsForEntity
                );
                var newSet = new System.Collections.Generic.HashSet<RecordId>(
        ...
        foreach (long entityId in affectedEntityIds)
        {
            ...
            await _stateRepo.SaveEntitySnapshot(entityId, existsNow, LanguageExt.Seq.createRange(nowRecords));
        }
